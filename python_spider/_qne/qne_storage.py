...
def save_comment_in_file():
    ...
    dataFrame.to_csv(path_or_buf="target.csv", encoding="utf-8-sig", index=False);

    # 将评论信息写入 txt 文件
    with open(file="target.txt", mode="a", encoding="utf-8") as f:
        for item in data:
            f.write(item[3] + "\n");
